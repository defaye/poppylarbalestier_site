    ...
    <div class="row mb-3">
      <div class="col mb-1">
        <div class="card">
          ...
          <draggable
            v-model="enabled"
            class="list-group list-group-flush dragarea"
            element="ul"
            :options="{ group: 'navigation' }"
          >
            <li class="list-group-item" v-for="page in enabled" :key="page.id">
              {{ page.title }}
            </li>
          </draggable>
        </div>
      ...
      <div class="col">
        <div class="card">
          ...
          <draggable
            ...
            :options="{ group: 'navigation' }"
          >
            <li class="list-group-item" v-for="page in disabled" :key="page.id">
              {{ page.title }}
            </li>
          </draggable>
        </div>
  ...
  methods: {
    async submit() {
      try {
        this.processing = true;
        this.errors.clear();
        const response = await axios.post("/api/admin/navigation", {
          items: this.enabled,
        });
        this.enabled = _.filter(response.data, (item) => item.id);
        this.disabled = _.filter(response.data, (item) => !item.id);
